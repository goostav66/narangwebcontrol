...
//$upload_dir = "http://hanjicds001.gabia.io/images/shop_photo";
	...
	if(in_array($detectedType, $allowedTypes))
	{
		...
		{	
			...
			if($sourcePath){
				...
				registerShopImage($connect, $idx, $photo_url);
	...
	if(in_array($detectedType, $allowedTypes))
	{
		...
		{	
			...
			if($sourcePath){
				...
				registerShopMenu($connect, $photo_url, $targetPath, $shop_idx);
			}else {
				echo "메뉴 등록에 실패하였습니다.";   
			}
		}
...
function registerShopImage($connect, $idx, $photo_url){
	$qry = "SELECT MAX(photo_order) FROM shop_photo WHERE shop_idx = '$idx'";
	...
	$qry2 = "INSERT INTO shop_photo (shop_idx, photo_url, photo_order) values ('$idx', '$photo_url', '$lastOrder')";
...
function RemoveDefaultPhoto($connect, $url){

}

function registerShopMenu($connect, $photo_url, $targetPath, $shop_idx){

	//echo " / menu / ";
	$menu_idx = $_POST['menu_idx'];
	$menu_type = $_POST['menu_type'];
	$menu_name = $_POST['menu_name'];
	$menu_infor= $_POST['menu_infor'];
	...
	if(isset($menu_idx) && !empty($menu_idx)){
		...
		if (!empty($targetPath) && isset($targetPath)){
			$qry = "SELECT menu_photo, shop_idx, menu_name
				FROM shop_menu
				WHERE idx = '$menu_idx'";
			...
			$qry2 = "UPDATE  `shop_menu` 
					SET 
					`menu_photo` = '$photo_url',
					`menu_type` = '$menu_type',
					`menu_name` = '$menu_name',
					`menu_infor` = '$menu_infor',
					`price` = '$price',
					`price_s` = '$price_s',
					`price_m` = '$price_m',
					`price_l` = '$price_l'
		...
		$qry = "INSERT INTO  `shop_menu` (
				...
				`menu_photo`)
				VALUES (
				'$shop_idx', '$menu_type', '$menu_name', '$menu_infor', '$price', '$price_s', '$price_m', '$price_l', '$photo_url')";
